...
- [x] (2026-07-01) M3 layout — the figure was unreadably small in the control column. Moved it to a wide resizable right `SidePanel`; the window grows to 1640×760 while shown and shrinks back when hidden (`GuiApp::live_chart_expanded`). Removed the first guide image column and renumbered steps (rehearsal-page guide → ①, control step → ②). `cargo check` clean, guarded release build OK. See Decision Log.
