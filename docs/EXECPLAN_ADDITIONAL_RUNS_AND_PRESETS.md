...
- 2026-06-16 (complete): Manual acceptance Scenarios A–D confirmed passing by the user against the live game. Both features meet their Purpose — a finished series can be extended into the same folder with unified output from both the 完了 panel and the idle "前回の結果" shortcut, 追加実行 and 続行 are mutually exclusive, and either run-count is settable with one 100/200/500/1000 preset tap. Shipped in release v0.4.0 (merged to `main` as 5de74d0).
