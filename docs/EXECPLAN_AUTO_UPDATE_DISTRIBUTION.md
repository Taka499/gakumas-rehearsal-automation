...
- [ ] M1 (in progress): `/release` skill retargeted to the dist repo (bot-authored, sha256 sidecar, privacy scrub step); first release published there. (completed 2026-07-08: `.claude/commands/release.md` rewritten — publishes to `tia-tools/releases` via `GH_TOKEN=$GAKUMAS_DIST_TOKEN` from `.env`, tags stay in this repo, new sha256-sidecar + privacy-scrub + author-verification steps; 2026-07-08 later: dist repo README written and pushed via the bot PAT — commit `2744bba`, author verified `tia-tools-bot`; remaining: publish the next release there (first dist-repo release).)
- [ ] M2 (in progress): Cloudflare Worker deployed; `https://tia.run/latest.json` and `/download/...` verified against the dist repo. (completed 2026-07-08: Worker source + `wrangler.toml` (custom_domain route) + deploy README written under `infra/worker/`; remaining: user runs `npx wrangler login` (project Cloudflare account) + `npx wrangler deploy` from `infra/worker/`, then the curl verifications — note `/latest.json` legitimately returns 502 until the first dist-repo release exists.)
- [ ] M3: Updater core in the app (`src/update/`): endpoint constants, manifest fetch with GitHub fallback, semver compare; unit-tested.
- [ ] M4: Install path + GUI surface: download → sha256 verify → exe rename-swap → resources replace → restart prompt; `.old` cleanup at startup.
- [ ] M5: End-to-end acceptance with a forced-old local build against the live endpoint.
