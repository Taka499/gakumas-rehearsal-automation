...
- [ ] M5: End-to-end acceptance with a forced-old local build against the live endpoint. (Ready to run now that v0.9.0 is live on the channel: set `Cargo.toml` version to `0.0.1` uncommitted → `scripts/build.ps1` → launch → expect header notice for v0.9.0 → アップデート → 再起動 → app reports v0.9.0, `config.json` mtime unchanged, `.exe.old` cleaned on following launch. Negative checks per the M5 milestone section. User-performed; agent cannot drive the elevated GUI.)
