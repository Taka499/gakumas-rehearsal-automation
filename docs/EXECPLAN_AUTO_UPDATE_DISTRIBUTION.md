...
- [ ] M1 (in progress): `/release` skill retargeted to the dist repo (bot-authored, sha256 sidecar, privacy scrub step); first release published there. (completed 2026-07-08: `.claude/commands/release.md` rewritten — publishes to `tia-tools/releases` via `GH_TOKEN=$GAKUMAS_DIST_TOKEN` from `.env`, tags stay in this repo, new sha256-sidecar + privacy-scrub + author-verification steps; remaining: publish the next release there (first dist-repo release) and write the dist repo's README, which is currently an empty stub.)
- [ ] M2: Cloudflare Worker deployed; `https://tia.run/latest.json` and `/download/...` verified against the dist repo.
