...
- [x] (2026-06-23) M2 — Solver robustness in `src/ocr/reconcile.rs`: (a) `candidates()` adds leading-digit-replacement (`1`/`2`) for an impossible ≥ `MAX_SCORE` seven-digit raw value; (b) `reconcile_stage` step 0 uses `plausible_floor` so an impossible raw slot does not reject a smaller true total; (c) the checksum search iterates `total_candidates` = literal + one-digit deletions, with a `+1` selection penalty on deletions; (d) `cost` charges `+1` for a changed leading digit, and recovery is now classified by `chosen == ocr_scores` (so a clean score under a deleted total is `Ok`, not `Repaired`). Tests added for the five real ≥3M cases and the comma-inserted-total false-flag case. `cargo test reconcile` → 33 passed.
- [x] (2026-06-23) M3 — Non-collision-prone fallback `resolve_without_checksum` in `reconcile.rs`: replaces the unconditional `solutions.is_empty() -> Flagged`. Collision-prone stays Flagged; otherwise a single-slot total-solve (`solve_single`) recovers dropped-leading-digit cases (`55172`→`855172`, `92118`→`892118`) corroborated by the bonus, else the raw read is `Ok` when the bonus is absent or corroborates, else `Flagged`. Tests added (recovery, transposed-total-bonus-corroborates `Ok`, bonus-contradicts `Flagged`, clean-stays-`Ok`). Full suite → 95 passed, 0 failed, 1 ignored.
- [x] (2026-06-23) M4 — Single-character digit-stream recovery in `reconstruct_from_digits` (k==1 first-part: 8-digit drops a spurious leading digit, impossible 7-digit replaces leading "1,"→digit with 1/2; physical-validity million-trade guard skipped for k==1). Test `test_reconstruct_single_char_split_iter400` added. End-to-end sweep over all 577 `20260623_232320` stages: **flagged stage-events 129→9, flagged iterations 121→9**; all 8 known-wrong stage-2 scores recovered to the correct values (Repaired); the 6 overlap fixtures still pass. Cross-reference added to `docs/EXECPLAN_OVERLAP_SCORE_RECOVERY.md`.
...
All four milestones landed 2026-06-23. Measured against Purpose:

- **False flags collapsed.** Re-OCR of all 577 readable stages in `target/release/output/20260623_232320/` drops flagged stage-events from **129 to 9** and flagged iterations from **121 to 9** — a >90% reduction. The 9 residual flags all retain a plausible single-character score conservatively marked `Flagged` (cases where the total survived only beyond a one-digit deletion *and* the bonus did not corroborate, e.g. a transposed total with a mis-read bonus); they are correctly surfaced for human review rather than silently trusted or mis-recovered.
- **All eight genuine errors recovered.** Iterations 207/267/304/349/363/400/472/521 now produce the corrected stage-2 scores (`892118`, `1172520`, `1177174`, `1117975`, `1115501`, `1110707`, `855172`, `1122517`), each `Repaired` and each satisfying the checksum exactly with bonus corroboration.
- **No regression.** The prior plan's six end-to-end overlap fixtures still pass; the full unit suite is 96 passed / 0 failed / 1 ignored.

What worked: hardening the checksum *inputs* (bonus `+`-required; total comma-deletion tolerance) plus a single-character total-solve turned out to address the overwhelming majority of cases with small, well-guarded additions — no rewrite of the solver core. Classifying recovery by `chosen == ocr_scores` (rather than raw cost) cleanly separated "confirmed a correct score under a noisy total" (`Ok`) from "edited a score" (`Repaired`).

Lessons / limits: the total's thousands comma OCR'ing as a digit is the single biggest source of noise on single-character stages; a one-digit-deletion checksum search handles an *insertion* but not a *transposition* (those fall back to bonus corroboration, else flag). The residual 9 flags could be reduced further by hardening the total region's preprocessing so the comma stops reading as a digit, or by a per-column bonus argmax cross-check (the prior plan's optional M5) — left as future work since the current behaviour is safe (flag, never silently wrong).

Not done / out of scope: the existing `20260623_232320` CSVs were intentionally left unpatched per user direction (see `DATA_QUALITY_NOTE.md` in that folder); only documented, not rewritten.
