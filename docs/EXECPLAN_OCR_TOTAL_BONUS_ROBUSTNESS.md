...
- [ ] M1 — Bonus `+`-anchor hardening: in `src/ocr/engine.rs::parse_single_number`, when `anchor_plus` is true and no `+` is present, return `None` instead of falling back to the whole string. Add unit tests proving `+65,575`→`65575`, a no-`+` crown-misread (`465575`)→`None`, and that a correctly-anchored read is unchanged.
