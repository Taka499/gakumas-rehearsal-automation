...
- [ ] M3 (inline dynamic layout) — replace the fixed 12-column `Grid` + right preview pane in `render_review_window_contents` with a per-row `ui.horizontal` whose cells size to the window width, and render the three per-stage UV crops beneath each stage's three cells **only** when that row is expanded. Manual acceptance.
...
(To be written as milestones land. Compare against Purpose: a user opening a finished session can expand a flagged row and read the character icons + printed scores inline under each stage, at a size that grows with the window, while editing the cell directly above.)
...
Reference geometry (sample `target/release/output/20260116_012805/screenshots/001_20260116_012814.png`, 721×1281):
    ...
    review_crop_adjust    = { top_extend: 0.05, bottom_extend: 0.0,
                              left_inset: 0.0,  right_inset: 0.22 }            # default
    review_crop_rect(.,0) ≈ { x: 0.0, y: 0.129, width: 0.78, height: 0.072 }  # portraits + scores, button trimmed
