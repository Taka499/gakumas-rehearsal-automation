...
- [ ] The user's interactive round-trip test run also failed, with NO test-harness output at all (not even `running 1 test`) — unexplained; awaiting a `--nocapture` re-run with full output. Note the harness output normally appears BETWEEN cargo's "Running unittests…" line and its "error: test failed" line; its total absence there suggests the test process died before the harness started.
- [ ] Final acceptance (manual, needs the interactive desktop): run `GAKUMAS_NO_MANIFEST=1 cargo test clipboard_roundtrip -- --ignored --nocapture` from a normal terminal; click-through both surfaces per Validation and Acceptance, including the failure toast.

## Surprises & Discoveries

- Observation: Processes spawned from the coding-agent shell session have no Windows clipboard access at all, so the clipboard round-trip test cannot be validated by an agent — it must be run from an interactive desktop terminal.
  Evidence: `arboard`'s `get_image` failed with "failed to read clipboard PNG data"; a `clipboard-win` diagnostic showed `GetClipboardData` failing with OS error 1418 ("Thread does not have a clipboard open") immediately after a successful `OpenClipboard`, with `EnumClipboardFormats` yielding nothing; even PowerShell's own `Set-Clipboard` failed with "Requested Clipboard operation did not succeed" (STA and non-STA alike, sandboxed and unsandboxed alike).
...
Milestone 1: the round-trip test passes — it sets a small known RGBA image on the clipboard, reads it back with arboard, and asserts dimensions and sampled pixels match. IMPORTANT: it must be run from an interactive desktop terminal, not from an agent/CI shell — those contexts have no clipboard access and the test fails there for environmental reasons (see Surprises & Discoveries).
