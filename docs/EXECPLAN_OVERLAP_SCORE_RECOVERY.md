...
- [x] (2026-06-29) M7 — **Stage total-OCR robustness** (branch `feature/stage-total-ocr-robustness`). Field verification of M6 (below) showed the dominant *residual* failure is no longer a candidate-model gap but the **stage total mis-OCR'ing**: a non-max slot correctly loses its leading "1" (a normal collision), but because it is not the max the bonus cannot pin it, and the total reads wrong (or `None`), so the exact checksum can't fire. The fix is **not stage-2-specific** (the total is read the same way for every stage). A Rust threshold sweep over the 6 residual totals was decisive: **no single `total_threshold` is best for all** — the current 210 reads it2/it174/it189 wrong, but t180 reads it2/it174 right and t220 reads it189 right (a "3" sharpens vs a comma-pixel drops at opposite cutoffs); it131/it194 read correctly at *every* threshold (their failure was the substitution gap, not the total); it183 is wrong at all thresholds (leading 3→5). Implemented: **(1) multi-threshold total retry** in `ocr_screenshot` (`src/ocr/mod.rs`) — when the primary read flags, re-OCR the total crop at `TOTAL_ALT_THRESHOLDS` (180/220/190/200/230/170/240) and adopt the first reading yielding a non-flagged **exact-checksum** recovery (no guessing; bonus still guards edited combos; runs only on the ~7% flagged stages, breaks on first success). **(2) leading "1"→"2" substitution candidate** in `slot_candidates` (`src/ocr/reconcile.rs`) — a `2,XXX,XXX` value also offers `1,XXX,XXX` (`BaseKind::Prepend`), disambiguated by the exact total (a genuine 2M keeps its raw value at cost 0). End-to-end over the M6 run's residual screenshots: **5 of 8 recovered** (it2/it174/it189 via total retry; it131/it194 via substitution), the other 3 correctly stay flagged (it183 total unreadable at any threshold; it6/it175 genuine multi-digit OCR loss). `cargo test` → 108 passed (incl. new `test_leading_two_to_one_substitution`, `test_genuine_two_million_not_swapped`; replay harness still 0 unexpected, no regression).
...
- Observation (M7 field verification, 2026-06-29, run `20260629_014136`, 200 iters): the multi-threshold retry + 1→2-swap held up (3 manual corrections, 2 correctly flagged true positives), but surfaced **one false positive** — a *pre-existing* hole, not an M7 regression. it200 stage 2 OCR'd as `[1383, 64377, 364]` (every score lost 3–4 digits) with a **correct** total `4,315,292`; the checksum search found nothing (no candidate can reach digits the OCR never produced), and `resolve_without_checksum` then returned **`Ok`** for the non-collision-prone, no-bonus stage — silently storing garbage. Root cause: reaching `resolve_without_checksum` means a *usable total was provided but unsatisfiable*, which for a multi-score row is definitive proof the scores are wrong, yet the `None => Ok` tail accepted them. Fix: a stage with ≥2 non-zero scores that reaches this fallback is now **Flagged**, never `Ok` (only a lone score, where a noisy total needn't condemn a clean value, stays `Ok`). Powerful side effect: flagging it200 (instead of short-circuiting to `Ok`) lets the **digit-stream fallback run** — which it never did before — and it then **fully recovers** `[1383068, 1377364, 1278247]` via the exact total checksum (`Repaired`). So the bug had been both storing garbage *and* suppressing the recovery that works. An independent total-checksum audit of the corrected CSV is otherwise clean (no other silent errors). Tests: `test_multi_score_unsatisfiable_total_flags_not_ok`; `cargo test` 109 passed, replay harness 0 unexpected.
  Evidence: `target/release/output/20260629_014136/{results.csv,session.log}` (it200 logged `scores=[1383, 64377, 364] total=Some(4315292) bonus=None (ok)`); end-to-end re-run post-fix yields the recovered values.
