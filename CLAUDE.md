...
- `docs/EXECPLAN_CHANGELOG_AND_JP_NOTES.md` - bilingual `CHANGELOG.md` (JP for users / EN for maintainers via `### English` subsections; format rules in the file's own header comment) embedded into the binary with `include_str!` (`src/gui/changelog.rs`) and shown in a new 更新履歴 header window (JP-only; NOT zip-shipped — the updater never overwrites existing root files, so a shipped file would go stale). Release notes are drafted in `CHANGELOG.md` BEFORE building (the exe embeds its own entry) and the release body's first paragraph is the one-line JAPANESE summary = the in-app update hover hint (release skill step 5; v0.9.1 body retro-edited to comply). Review window default filter → `flagged` only. Complete (M1–M5; user click-through 2026-07-13; shipped in v0.10.0, the first release through the CHANGELOG-first flow, JP manifest `notes` verified live).
