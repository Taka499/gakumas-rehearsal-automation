...
- `docs/EXECPLAN_CHANGELOG_AND_JP_NOTES.md` - bilingual `CHANGELOG.md` (JP for users / EN for maintainers via `### English` subsections) embedded into the binary with `include_str!` and shown in a new 更新履歴 header window (JP-only; NOT zip-shipped — the updater never overwrites existing root files, so a shipped file would go stale); Japanese update hover hint by convention (release body's first paragraph = one-line JP summary; release skill amended, v0.9.1 body retro-edited); review window default filter → `flagged` only. In progress; see its Progress section.
