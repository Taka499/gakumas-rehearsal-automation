...
- `docs/EXECPLAN_IMAGE_COPY_TO_CLIPBOARD.md` - right-click-to-copy for GUI images (live box plot + review stage crops): direct copy at native resolution via `arboard` (box plot re-rendered from cached `DataSetStats`; stage crop re-cropped from the screenshot PNG on disk), with a fade-in/out "📋 コピーしました" toast overlaid on the clicked image (red on failure). Built as a reusable `copy_on_right_click` wrapper for future image surfaces. Code-complete (M1–M3, 2026-07-07; branch `feature/image-copy-to-clipboard`; 124 unit tests pass, release build links). Manual acceptance pending — including the clipboard round-trip test, which MUST be run from an interactive terminal (agent shells have no clipboard access; see the plan's Surprises).
