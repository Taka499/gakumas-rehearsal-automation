...
- `docs/EXECPLAN_FEEDBACK_FORM.md` - in-app feedback form: a 「フィードバック」 header button opens a floating egui form (message + bug/request/other category; bug reveals a session-log picker, newest preselected, 添付しない available) that POSTs to `https://rehearsal-automation.tia.run/feedback`; the Worker (size-capped, rate-limited per ADR-0012 daily IP hash) creates a labeled issue in the private `tia-tools/feedback` repo via a NEW issues-only fine-grained PAT (never the release PAT — per `docs/adr/0015`). Log travels inline, tail-truncated to ~60KB (GitHub: no issue-attachment API, 65,536-char body cap). Designed 2026-07-11 (grilling interview); not started. See its Progress section.
