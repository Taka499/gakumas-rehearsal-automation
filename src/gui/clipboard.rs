...
mod tests {
    ...
    /// from agent/service contexts have no clipboard access (GetClipboardData
    /// fails with OS error 1418 even after a successful OpenClipboard; even
    /// PowerShell's Set-Clipboard fails there) — and note that in such
    /// contexts `set_image` "succeeds" silently, so a passing *write* proves
    /// nothing without this read-back.
    #[test]
    #[ignore]
    fn clipboard_roundtrip() {
        // Opaque test pattern: alpha 255 everywhere, because Windows DIBV5
        // round-trips opaque pixels losslessly while transparent ones may
        // come back premultiplied.
        let (w, h) = (4u32, 2u32);
        ...
        let img = clipboard
            .get_image()
            .expect("an image should be on the clipboard");
        assert_eq!((img.width, img.height), (w as usize, h as usize));
